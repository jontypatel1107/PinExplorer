...
const getPincodeByQuery = async (req, res) => {
  try {
...
const getPincodeByParam = async (req, res) => {
  try {
...
const getTaluksAdvanced = async (req, res) => {
  try {
    const { state, district } = req.params;
...
const getLegacyStates = async (req, res) => {
  try {
    const states = await Pincode.distinct("stateName");
    const uniqueStates = [...new Set(states.map((s) => s?.trim()).filter(Boolean))].sort();
...
const getLegacyStateByName = async (req, res) => {
  try {
